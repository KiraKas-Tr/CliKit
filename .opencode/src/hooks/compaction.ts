 ...
 */
export function readBeadsState(projectDir: string): BeadsState | undefined {
    ...
    try {
      ...
      for (const lockFile of lockFiles) {
        try {
          ...
          if (lock.path) reservedFiles.push(lock.path);
          if (lock.agent) state.agentId = lock.agent;
        } catch {
  ...
  try {
    ...
    if (output.trim()) {
      ...
      if (Array.isArray(tasks) && tasks.length > 0) {
        state.currentTask = tasks[0].title || tasks[0].t;
 ...
 */
export function readMemoryRefs(projectDir: string, limit: number = 10): MemoryRef[] {
