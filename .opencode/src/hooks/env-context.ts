...
export function collectEnvInfo(cwd: string, config?: EnvContextConfig): EnvInfo {
  const info: EnvInfo = {
  ...
  if (config?.include_git !== false) {
    info.git = getGitInfo(cwd);
  }

  if (config?.include_package !== false) {
    info.package = getPackageInfo(cwd);
  }

  if (config?.include_structure !== false) {
    info.structure = getTopLevelStructure(cwd, config?.max_depth ?? 2);
  }
