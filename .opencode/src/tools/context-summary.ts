...
export function contextSummary(params: ContextSummaryParams = {}): ContextSummaryResult {
  ...
  for (const obs of observations) {
    try {
      const read = JSON.parse(obs.files_read || "[]") as string[];
      const modified = JSON.parse(obs.files_modified || "[]") as string[];
